...
parser.add_argument('name', help='Game name and version')
...
class Project:
    def __init__(self, game_name: str, *, platform: Platform, delinks_json: Any | None):
        self.game_name = game_name
        '''Game name and version'''
        self.game_config = config_path / game_name
        '''Root directory for dsd configs'''

        if not self.game_config.is_dir():
            print(f"Version '{game_name}' not recognized")
            exit(1)
        ...
        self.game_build = build_path / game_name
        '''Path to build directory'''
        self.game_extract = extract_path / game_name
    ...
    def baserom(self) -> Path:
        return extract_path / f'{self.game_name}.nds'
...
def main():
    ...
    if can_run_dsd:
        out = subprocess.run([
            ...
            "json",
            "delinks",
            "--config-path", config_path / args.name / "arm9" / "config.yaml"
    ...
    project = Project(args.name, platform=platform, delinks_json=delinks_json)
    ...
    with build_ninja_path.open("w") as file:
        ...
        n.rule(
            name="mwld",
            command=f'{WINE} "{project.ld}" {LD_FLAGS} $extra_ld_flags @$objects_file $lcf_file -o $out'
        )
        n.newline()
...
def add_download_tool_builds(n: ninja_syntax.Writer, project: Project):
    ...
    if args.compiler is None:
        downloads.extend([project.cc, project.ld])
        n.build(
            rule="download_tool",
            outputs=[project.cc, project.ld],
            variables={
                "tool": "mwccarm",
                "tag": "latest",
                "path": str(tools_path),
...
def add_mwld_builds(n: ninja_syntax.Writer, project: Project):
...
def add_mwcc_builds(n: ninja_syntax.Writer, project: Project, mwcc_implicit: list[str]):
    for source_file in get_c_cpp_files([src_path, libs_path]):
        ...
        n.build(
            ...
            variables={
                "game_version": project.game_name,
