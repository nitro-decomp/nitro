...
def main():
    ...
    with tempfile.NamedTemporaryFile(delete=True, suffix=suffix) as tmp_file:
        ...
        # Run preprocessor
        try:
            ctx: str = subprocess.check_output([ # type: ignore
                'gcc',
                '-E', '-P', '-fworking-directory', '-undef', '-dD',
                *CXX_FLAGS,
                tmp_file.name
            ], cwd=root_dir, encoding=args.encoding)
            ctx = str(ctx) # type: ignore
    ...
    for i in reversed(range(len(lines))):
        if (
            lines[i].startswith('#define __cplusplus')
            or lines[i].startswith('#define __STDC_HOSTED__')
            or lines[i].startswith('#define __STDC__')
            or lines[i].startswith('#define __STDC_VERSION__')
        ):
            lines.pop(i)
