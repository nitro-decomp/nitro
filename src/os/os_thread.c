...
static void OS_func_0206b8fc(OSThread *thread, void *arg);
static void OS_func_0206b958(void *);
static void OS_func_0206b990(void);
...
static u32 OS_func_0206b3e0(void) {
    return ++sThreadCount;
}

static void OS_func_0206b3f8(OSLinkedList *list, OSThread *thread) {
    OSThread *it;
...
static OSThread *OS_func_0206b470(OSLinkedList *list) {
...
static OSThread *OS_func_0206b4a4(OSLinkedList *list, OSThread *thread) {
    OSThread *iter;
    OSThread *next;
...
static void OS_func_0206b528(OSThread *thread) {
...
static void OS_func_0206b588(OSThread *param_1) {
    OSThread *iter = data_0216a1a8;
    OSThread *prev = NULL;
...
static void OS_func_0206b5d0(void) {
    OSThread *pOVar1;
    OSThread *pOVar2;
...
void OS_CreateThread(OSThread *thread, void (*threadFunc)(void *arg), void *arg, void *stackHi, u32 stackSize,
    ...
    u32 uVar2      = OS_func_0206b3e0();
    thread->prio   = prio;
    thread->unk_6c = uVar2;
    thread->unk_64 = 0;
    thread->unk_74 = 0;
    OS_func_0206b528(thread);
    ...
    *(u32 *) thread->stackLo       = 0x7bf9dd5b;
    ...
    thread->arg    = arg;
    thread->unk_3c = OS_func_0206b8dc;
...
void OS_func_0206b8dc(void) {
    ...
    OS_func_0206b8fc(data_0216a1a4, NULL);
}

static void OS_func_0206b8fc(OSThread *thread, void *arg) {
    if (data_0216a198 != 0) {
        OS_func_0206be20(thread, OS_func_0206b958, data_0216a198);
        thread->arg = arg;
        thread->unk_00 |= 0x80;
        thread->unk_64 = 1;
        OS_func_0206bef0(thread);
    } else {
        OS_func_0206b958(arg);
    }
}

static void OS_func_0206b958(void *arg) {
